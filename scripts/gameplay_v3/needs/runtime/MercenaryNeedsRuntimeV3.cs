...
{
    private MercenaryNeedsSessionV3? _needs;private MercenarySessionV3? _mercenaries;private RestWorkCoordinatorV3? _rest;private EatingWorkCoordinatorV3? _eating;private double _accumulator;public int TickCount{get;private set;}
    ...
    private void TryStartNeedAction(string id){foreach(NeedActionKindV3 action in NeedActionPriorityV3.GetCandidates(_needs!.Hunger.GetHunger(id),_needs.Fatigue.GetValue(id),_needs.HungerConfig,_needs.Settings)){if(action==NeedActionKindV3.Eat&&_eating?.TryIssueAuto(id,out _)==true)return;if(action==NeedActionKindV3.Rest&&_rest?.TryAutoRest(id)==true)return;}}
}
