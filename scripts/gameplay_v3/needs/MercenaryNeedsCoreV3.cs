...
public sealed class MercenaryNeedsSessionV3
{
    ...
    public MercenaryNeedsSessionV3(long revision){SessionRevision=revision;}public long SessionRevision{get;}public MercenaryFatigueSettingsV3 Settings{get;}=new();public MercenaryFatigueRegistryV3 Fatigue{get;}=new();public RestAssignmentRegistryV3 Assignments{get;}=new();public RestReservationRegistryV3 Reservations{get;}=new();public MercenaryNeedsDiagnosticsV3 Diagnostics{get;}=new();public int ActiveRestCount=>_activeRestByMercenary.Count;
    public void EnsureMercenaries(MercenaryRegistryV3 registry){foreach(string id in registry.GetAllMercenaryIds())Fatigue.GetOrCreate(id);}public float MovementMultiplier(string id)=>MercenaryFatiguePolicyV3.MovementMultiplier(Fatigue.GetValue(id));public float WorkMultiplier(string id)=>MercenaryFatiguePolicyV3.WorkMultiplier(Fatigue.GetValue(id));public bool CanStartWork(string id,WorkTypeV3 type,out string reason){if(type==WorkTypeV3.Rest){reason=string.Empty;return true;}if(Fatigue.GetValue(id)>=Settings.WorkBlockThreshold){Diagnostics.BlockedWorkCount++;reason="TooExhausted";return false;}reason=string.Empty;return true;}
