...
public partial class ConstructionUiV3 : Godot.Control
{
    ...
    public event Action<bool>? WoodenWallToolChanged;
    ...
    private void CreateUi()
    {
        ...
        _tray=new PanelContainer{Name="ConstructionTray",Visible=false,MouseFilter=MouseFilterEnum.Stop,CustomMinimumSize=new Vector2(390,58)};ConfigureRect(_tray,54,-76,444,-12);_tray.AddThemeStyleboxOverride("panel",CreatePanelStyle());AddChild(_tray);
    ...
    private void OnDemolitionButtonPressed(){bool active=_demolitionButton?.ButtonPressed==true;if(active){_designation?.SetMode(StockpileDesignationModeV3.None);SetWallTool(false);}SetDemolitionTool(active);GetViewport().SetInputAsHandled();}
    public void SetWallTool(bool active){if(active&&_demolitionButton?.ButtonPressed==true)SetDemolitionTool(false);if(_wallButton!=null)_wallButton.ButtonPressed=active;ActiveConstructionTool=active?"WoodenWall":_demolitionButton?.ButtonPressed==true?"Demolition":"-";LastConstructionUiAction=active?"Wooden Wall selected":"Wooden Wall cancelled";WoodenWallToolChanged?.Invoke(active);RefreshManagerState();}
    public void SetDemolitionTool(bool active){if(_demolitionButton!=null)_demolitionButton.ButtonPressed=active;ActiveConstructionTool=active?"Demolition":_wallButton?.ButtonPressed==true?"WoodenWall":"-";LastConstructionUiAction=active?"Demolition selected":"Demolition cancelled";DemolitionToolChanged?.Invoke(active);RefreshManagerState();}
    private void OnDesignationModeChanged(StockpileDesignationModeV3 mode){if(mode!=StockpileDesignationModeV3.None){if(_wallButton?.ButtonPressed==true)SetWallTool(false);if(_demolitionButton?.ButtonPressed==true)SetDemolitionTool(false);}if(_stockpileButton!=null)_stockpileButton.ButtonPressed=mode==StockpileDesignationModeV3.Add;if(_removeButton!=null)_removeButton.ButtonPressed=mode==StockpileDesignationModeV3.Remove;ActiveConstructionTool=mode switch{StockpileDesignationModeV3.Add=>"StockpileAdd",StockpileDesignationModeV3.Remove=>"StockpileRemove",_=>_demolitionButton?.ButtonPressed==true?"Demolition":_wallButton?.ButtonPressed==true?"WoodenWall":"-"};LastConstructionUiAction=mode switch{StockpileDesignationModeV3.Add=>"Stockpile Add entered",StockpileDesignationModeV3.Remove=>"Stockpile Remove entered",_=>"Designation cancelled"};RefreshManagerState();}
    private void SetTrayOpen(bool open,string action){if(_tray==null)return;_tray.Visible=open;if(!open){SetWallTool(false);SetDemolitionTool(false);}LastConstructionUiAction=open?"Construction tray opened":action=="escape"?"Construction tray closed":"Construction tray closed";if(_toggleButton!=null)_toggleButton.ButtonPressed=open;RefreshManagerState();if(open)GD.Print("[ConstructionUiV3] Construction tray opened");else GD.Print("[ConstructionUiV3] Construction tray closed");}
