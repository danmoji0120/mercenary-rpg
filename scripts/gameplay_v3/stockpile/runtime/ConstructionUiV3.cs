...
public partial class ConstructionUiV3 : Godot.Control
{
    ...
    private void CreateUi()
    {
        ...
        _tray=new PanelContainer{Name="ConstructionTray",Visible=false,MouseFilter=MouseFilterEnum.Stop,CustomMinimumSize=new Vector2(600,58)};ConfigureRect(_tray,54,-76,654,-12);_tray.AddThemeStyleboxOverride("panel",CreatePanelStyle());AddChild(_tray);
        ...
        _wallButton=new Button{Name="WoodenWallToolButton",Text="▥  나무 벽",ToggleMode=true,FocusMode=Godot.Control.FocusModeEnum.None,CustomMinimumSize=new Vector2(104,30),MouseFilter=MouseFilterEnum.Stop};_wallButton.Pressed+=OnWallButtonPressed;tools.AddChild(_wallButton);ApplyButtonStyle(_wallButton,104,30);
        _bedButton=new Button{Name="BasicBedToolButton",Text="▰  간이 침대",ToggleMode=true,FocusMode=Godot.Control.FocusModeEnum.None,CustomMinimumSize=new Vector2(104,30),MouseFilter=MouseFilterEnum.Stop};_bedButton.Pressed+=OnBedButtonPressed;tools.AddChild(_bedButton);ApplyButtonStyle(_bedButton,104,30);
    ...
    private void OnWallButtonPressed(){bool active=_wallButton?.ButtonPressed==true;if(active)_designation?.SetMode(StockpileDesignationModeV3.None);SetWallTool(active);GetViewport().SetInputAsHandled();}
    private void OnBedButtonPressed(){bool active=_bedButton?.ButtonPressed==true;if(active)_designation?.SetMode(StockpileDesignationModeV3.None);SetBedTool(active);GetViewport().SetInputAsHandled();}
    private void OnFarmButtonPressed(){bool active=_farmButton?.ButtonPressed==true;if(active)_designation?.SetMode(StockpileDesignationModeV3.None);SetFarmTool(active);GetViewport().SetInputAsHandled();}
    private void OnDemolitionButtonPressed(){bool active=_demolitionButton?.ButtonPressed==true;if(active)_designation?.SetMode(StockpileDesignationModeV3.None);SetDemolitionTool(active);GetViewport().SetInputAsHandled();}
    public void SetWallTool(bool active){if(active){if(_demolitionButton?.ButtonPressed==true)SetDemolitionTool(false);if(_bedButton!=null)_bedButton.ButtonPressed=false;if(_farmButton?.ButtonPressed==true)SetFarmTool(false);}if(_wallButton!=null)_wallButton.ButtonPressed=active;RefreshActiveConstructionTool();LastConstructionUiAction=active?"Wooden Wall selected":"Wooden Wall cancelled";ConstructionToolChanged?.Invoke(ConstructionPlacementToolKindV3.WoodenWall,active);RefreshManagerState();}
    public void SetBedTool(bool active){if(active){if(_demolitionButton?.ButtonPressed==true)SetDemolitionTool(false);if(_wallButton!=null)_wallButton.ButtonPressed=false;if(_farmButton?.ButtonPressed==true)SetFarmTool(false);}if(_bedButton!=null)_bedButton.ButtonPressed=active;RefreshActiveConstructionTool();LastConstructionUiAction=active?"Basic Bed selected":"Basic Bed cancelled";ConstructionToolChanged?.Invoke(ConstructionPlacementToolKindV3.BasicBed,active);RefreshManagerState();}
    public void SetFarmTool(bool active){if(active){if(_demolitionButton?.ButtonPressed==true)SetDemolitionTool(false);if(_wallButton!=null)_wallButton.ButtonPressed=false;if(_bedButton!=null)_bedButton.ButtonPressed=false;}if(_farmButton!=null)_farmButton.ButtonPressed=active;RefreshActiveConstructionTool();LastConstructionUiAction=active?"Potato Farm selected":"Potato Farm cancelled";FarmToolChanged?.Invoke(active);RefreshManagerState();}
    public void SetDemolitionTool(bool active){if(active){if(_wallButton!=null)_wallButton.ButtonPressed=false;if(_bedButton!=null)_bedButton.ButtonPressed=false;if(_farmButton?.ButtonPressed==true)SetFarmTool(false);}if(_demolitionButton!=null)_demolitionButton.ButtonPressed=active;RefreshActiveConstructionTool();LastConstructionUiAction=active?"Demolition selected":"Demolition cancelled";DemolitionToolChanged?.Invoke(active);RefreshManagerState();}
    private void OnDesignationModeChanged(StockpileDesignationModeV3 mode){if(mode!=StockpileDesignationModeV3.None){SetWallTool(false);SetBedTool(false);SetFarmTool(false);SetDemolitionTool(false);}if(_stockpileButton!=null)_stockpileButton.ButtonPressed=mode==StockpileDesignationModeV3.Add;if(_removeButton!=null)_removeButton.ButtonPressed=mode==StockpileDesignationModeV3.Remove;RefreshActiveConstructionTool();LastConstructionUiAction=mode switch{StockpileDesignationModeV3.Add=>"Stockpile Add entered",StockpileDesignationModeV3.Remove=>"Stockpile Remove entered",_=>"Designation cancelled"};RefreshManagerState();}
    private void RefreshActiveConstructionTool(){ActiveConstructionTool=_designation?.Mode switch{StockpileDesignationModeV3.Add=>"StockpileAdd",StockpileDesignationModeV3.Remove=>"StockpileRemove",_=>_demolitionButton?.ButtonPressed==true?"Demolition":_farmButton?.ButtonPressed==true?"PotatoFarm":_bedButton?.ButtonPressed==true?"BasicBed":_wallButton?.ButtonPressed==true?"WoodenWall":"-"};}
    private void SetTrayOpen(bool open,string action){if(_tray==null)return;_tray.Visible=open;if(!open){SetWallTool(false);SetBedTool(false);SetFarmTool(false);SetDemolitionTool(false);}LastConstructionUiAction=open?"Construction tray opened":action=="escape"?"Construction tray closed":"Construction tray closed";if(_toggleButton!=null)_toggleButton.ButtonPressed=open;RefreshManagerState();if(open)GD.Print("[ConstructionUiV3] Construction tray opened");else GD.Print("[ConstructionUiV3] Construction tray closed");}
