...
public partial class ConstructionUiV3 : Godot.Control
{
    ...
    private void CreateUi()
    {
        ...
        _tray=new PanelContainer{Name="ConstructionTray",Visible=false,MouseFilter=MouseFilterEnum.Stop,CustomMinimumSize=new Vector2(300,58)};ConfigureRect(_tray,54,-76,354,-12);_tray.AddThemeStyleboxOverride("panel",CreatePanelStyle());AddChild(_tray);
    ...
    public void SetWallTool(bool active){if(_wallButton!=null)_wallButton.ButtonPressed=active;ActiveConstructionTool=active?"WoodenWall":"-";LastConstructionUiAction=active?"Wooden Wall selected":"Wooden Wall cancelled";WoodenWallToolChanged?.Invoke(active);RefreshManagerState();}
    private void OnDesignationModeChanged(StockpileDesignationModeV3 mode){if(mode!=StockpileDesignationModeV3.None&&_wallButton?.ButtonPressed==true)SetWallTool(false);if(_stockpileButton!=null)_stockpileButton.ButtonPressed=mode==StockpileDesignationModeV3.Add;if(_removeButton!=null)_removeButton.ButtonPressed=mode==StockpileDesignationModeV3.Remove;ActiveConstructionTool=mode switch{StockpileDesignationModeV3.Add=>"StockpileAdd",StockpileDesignationModeV3.Remove=>"StockpileRemove",_=>_wallButton?.ButtonPressed==true?"WoodenWall":"-"};LastConstructionUiAction=mode switch{StockpileDesignationModeV3.Add=>"Stockpile Add entered",StockpileDesignationModeV3.Remove=>"Stockpile Remove entered",_=>"Designation cancelled"};RefreshManagerState();}
    private void SetTrayOpen(bool open,string action){if(_tray==null)return;_tray.Visible=open;if(!open)SetWallTool(false);LastConstructionUiAction=open?"Construction tray opened":action=="escape"?"Construction tray closed":"Construction tray closed";if(_toggleButton!=null)_toggleButton.ButtonPressed=open;RefreshManagerState();if(open)GD.Print("[ConstructionUiV3] Construction tray opened");else GD.Print("[ConstructionUiV3] Construction tray closed");}
