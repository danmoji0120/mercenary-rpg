...
public partial class StockpileDesignationControllerV3 : Node
{
    ...
    private IReadOnlyList<GlobalCellCoord> BuildCells(Vector2 a,Vector2 b){List<GlobalCellCoord> result=new();Vector2 wa=GetViewport().GetCanvasTransform().AffineInverse()*a,wb=GetViewport().GetCanvasTransform().AffineInverse()*b;Vector2I ca=_grid!.WorldToCell(wa),cb=_grid.WorldToCell(wb);int minX=Math.Min(ca.X,cb.X),maxX=Math.Max(ca.X,cb.X),minY=Math.Min(ca.Y,cb.Y),maxY=Math.Max(ca.Y,cb.Y);for(int y=minY;y<=maxY&&result.Count<MaxCellsPerDrag;y++)for(int x=minX;x<=maxX&&result.Count<MaxCellsPerDrag;x++){Vector2I cell=new(x,y);GlobalCellCoord global=new(cell);if(!_query!.IsInsideWorld(cell)||!_query.IsWalkable(cell)||_resources!.Nodes.ContainsCell(cell))continue;if(Mode==StockpileDesignationModeV3.Add&&_session!.Zones.IsStockpileCell(global))continue;result.Add(global);}return result.AsReadOnly();}
