...
public partial class MercenaryMovementRuntimeV3 : Node
{
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        foreach(MercenaryMoveOrderV3 order in _control.Commands.GetPendingOrders())_scheduler.Enqueue(order.PathRequest);
        foreach(MercenaryMovementRequestV3 request in _control.ExternalMovements.GetPending())_scheduler.Enqueue(request.PathRequest);
        ...
        foreach(MercenaryPathResultV3 result in results)ApplyResult(result);
        StartReadyOrders();StartReadyExternalMovements();_movement.Tick((float)delta,_control,_query);CompleteExternalMovements();UpdateMovingViews();SnapCompletedViews();
    }
    ...
    public int PendingPathRequestCount=>_scheduler.PendingCount;public int ActivePathSearchCount=>_scheduler.ActiveCount;
}
