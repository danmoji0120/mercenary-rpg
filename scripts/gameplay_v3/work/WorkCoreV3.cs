...
public enum WorkTypeV3{Gathering,Hauling,Construction,Demolition,Rest,Eating,Sowing,Harvesting,Production}
public enum WorkTargetKindV3{ResourceNode,GroundResourceStack,ConstructionBlueprint,Structure,RestSlot,FarmCell,FloorBlueprint,FloorCell,GroundEquipment,StoredEquipment}
...
{
    private readonly CompanySessionV3 _companies;private readonly MercenarySessionV3 _mercenaries;private readonly ResourceSessionV3 _resources;private readonly MercenaryControlSessionV3 _control;private readonly Dictionary<string,WorkRequestV3> _requests=new(StringComparer.Ordinal);private readonly Dictionary<string,MercenaryWorkAssignmentV3> _assignmentsByMercenary=new(StringComparer.Ordinal);private readonly Dictionary<string,MercenaryWorkExecutionStateV3> _executionsByRequest=new(StringComparer.Ordinal);private readonly Queue<string> _history=new();private WorkToolReservationSessionV3? _tools;private EquipmentLoadoutRuntimeV3? _equipmentLoadouts;private long _revision;
    public MercenaryWorkSessionV3(long sessionRevision,CompanySessionV3 companies,MercenarySessionV3 mercenaries,ResourceSessionV3 resources,StockpileSessionV3 stockpiles,MercenaryControlSessionV3 control){SessionRevision=sessionRevision;_companies=companies;_mercenaries=mercenaries;_resources=resources;Stockpiles=stockpiles;_control=control;SourceStackReservations=new(resources.AmountReservations,resources.GroundStacks);}
    private Action<string>? _externalWorkSupersede;private Func<string,WorkTypeV3,(bool Allowed,string Reason)>? _startPolicy;public long SessionRevision{get;}public StockpileSessionV3 Stockpiles{get;}public ResourceReservationRegistryV3 Reservations{get;}=new();public MercenaryWorkDiagnosticsV3 Diagnostics{get;}=new();public int ActiveWorkRequestCount=>_executionsByRequest.Count+_haulingExecutionsByRequest.Count+_equipmentExecutions.Count;public int ActiveAssignmentCount=>_assignmentsByMercenary.Count;public int ActiveReservationCount=>Reservations.Count+SourceStackReservations.Count+Stockpiles.CellReservations.Count;public void AttachExternalWorkSupersede(Action<string> callback)=>_externalWorkSupersede=callback;public void AttachStartPolicy(Func<string,WorkTypeV3,(bool Allowed,string Reason)> policy)=>_startPolicy=policy;public void AttachToolReservations(WorkToolReservationSessionV3 tools)=>_tools=tools;public void AttachEquipmentLoadouts(EquipmentLoadoutRuntimeV3 loadouts){if(loadouts.SessionRevision==SessionRevision)_equipmentLoadouts=loadouts;}public bool CanStartFor(string id,WorkTypeV3 type,out string reason){var result=_startPolicy?.Invoke(id,type)??(true,string.Empty);reason=result.Reason;return result.Allowed;}public void NotifyExternalWorkSupersede(string mercenaryId)=>_externalWorkSupersede?.Invoke(mercenaryId);
    ...
    public bool IsBoundTo(MercenaryControlSessionV3 control,ResourceSessionV3 resources,MercenarySessionV3 mercenaries)=>ReferenceEquals(_control,control)&&ReferenceEquals(_resources,resources)&&ReferenceEquals(_mercenaries,mercenaries);
    ...
    {request=null;if(currentSession!=SessionRevision){reason="InvalidSession";return false;}if(!_companies.CanPlayerControlCompany(issuer,companyId)){reason="OwnershipDenied";return false;}if(!_resources.Nodes.TryGet(nodeId,out ResourceNodeStateV3? node)||node==null){reason="InvalidResourceNode";return false;}if(node.IsDepleted){reason="ResourceDepleted";return false;}if(Reservations.IsReserved(nodeId)){reason="ResourceAlreadyReserved";return false;}if(!GatheringWorkerSelectionServiceV3.TrySelect(selected,node.Cell,_mercenaries,issuer,out string worker,out reason))return false;if(!CanStartFor(worker,WorkTypeV3.Gathering,out reason))return false;if(!_mercenaries.Registry.TryGetMercenary(worker,out MercenaryProfileV3? profile,out MercenaryStateV3? state)||profile==null||state==null||state.CompanyId!=companyId){reason="InvalidMercenary";return false;}IReadOnlyList<GlobalCellCoord> approaches=GatheringApproachCellServiceV3.GetCandidates(state.CurrentCell,node.Cell,query);if(approaches.Count==0){reason="NoApproachCells";return false;}
        NotifyExternalWorkSupersede(worker);if(_assignmentsByMercenary.TryGetValue(worker,out MercenaryWorkAssignmentV3? old))Terminal(old.WorkRequestId,WorkRequestStatusV3.Superseded,"SupersededByNewWork");long revision=++_revision;DateTime now=DateTime.UtcNow;WorkRequestV3 created=new(WorkRequestIdFactoryV3.Create(),companyId,nodeId,node.Cell,worker,revision,now);ResourceReservationV3 reservation=new(nodeId,created.WorkRequestId,worker,companyId,revision,now);if(!Reservations.TryReserve(reservation,out reason))return false;float toolMultiplier=_equipmentLoadouts?.GetGatheringWorkSpeedMultiplier(worker)??1f;if(toolMultiplier<=1.0001f)_tools?.TryReserveForGathering(companyId,worker,created.WorkRequestId,node.NodeType,out toolMultiplier);_control.SupersedeDirectMovementForWork(worker);MercenaryWorkAssignmentV3 assignment=new(WorkAssignmentIdFactoryV3.Create(),created,now);GatheringWorkCalculationV3 calculation=GatheringWorkCalculatorV3.Calculate(profile);MercenaryWorkExecutionStateV3 execution=new(created,approaches,calculation.RequiredWorkSeconds/toolMultiplier);created.Status=WorkRequestStatusV3.Assigned;_requests.Add(created.WorkRequestId,created);_assignmentsByMercenary.Add(worker,assignment);_executionsByRequest.Add(created.WorkRequestId,execution);Diagnostics.LastWorkRequestId=created.WorkRequestId;request=created;reason=string.Empty;return true;}
    ...
    public int CancelAllTransientWork(string reason)
    {
        List<string> requests=new(_assignmentsByMercenary.Count);
        foreach(MercenaryWorkAssignmentV3 assignment in _assignmentsByMercenary.Values)requests.Add(assignment.WorkRequestId);
        int cancelled=0;
        foreach(string requestId in requests)if(Terminal(requestId,WorkRequestStatusV3.Cancelled,reason))cancelled++;
        return cancelled;
    }
