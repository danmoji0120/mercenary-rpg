...
{
    private readonly Dictionary<string,GroundStackReservationV3> _byStack=new(StringComparer.Ordinal); public int Count=>_byStack.Count;
    public bool TryReserve(GroundStackReservationV3 reservation,out string reason){if(_byStack.TryGetValue(reservation.ResourceStackId,out GroundStackReservationV3? old)){if(old.WorkRequestId==reservation.WorkRequestId){reason=string.Empty;return true;}reason="SourceAlreadyReserved";return false;}_byStack.Add(reservation.ResourceStackId,reservation);reason=string.Empty;return true;}
    ...
    public bool TryRelease(string id,string request)=>_byStack.TryGetValue(id,out GroundStackReservationV3? r)&&r.WorkRequestId==request&&_byStack.Remove(id);
    public int ReleaseByWorkRequest(string request){List<string> ids=new();foreach(var pair in _byStack)if(pair.Value.WorkRequestId==request)ids.Add(pair.Key);foreach(string id in ids)_byStack.Remove(id);return ids.Count;}
    public int ReleaseByMercenary(string mercenary){List<string> ids=new();foreach(var pair in _byStack)if(pair.Value.MercenaryId==mercenary)ids.Add(pair.Key);foreach(string id in ids)_byStack.Remove(id);return ids.Count;}
    public void Clear()=>_byStack.Clear();
}
...
public sealed partial class MercenaryWorkSessionV3
{
    ...
    public GroundStackReservationRegistryV3 SourceStackReservations{get;}=new();public MercenaryCarryRegistryV3 Carries{get;}=new();public HaulingDiagnosticsV3 HaulingDiagnostics{get;}=new();
