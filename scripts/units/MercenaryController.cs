...
public partial class MercenaryController : Node2D
{
	...
	public MercenaryProfile Profile { get; private set; } = MercenaryProfileProvider.CreateFallbackProfile();
	public MercenaryInventory Inventory { get; } = new();
	...
	public string CurrentLogisticsValidationWarning => _logisticsValidationWarning;
	...
	private string _logisticsValidationWarning = "";
	...
	public override void _Ready()
	{
		...
		_nameLabel = new Label
		{
			...
			Position = new Vector2(-90.0f, -56.0f),
			Size = new Vector2(180.0f, 42.0f),
	...
	{
		Initialize(mercenaryName, spawnPosition, moveSpeed, null);
	}

	public void Initialize(string mercenaryName, Vector2 spawnPosition, float moveSpeed, MercenaryProfile? profile)
	{
		MercenaryProfile effectiveProfile = profile ?? MercenaryProfileProvider.CreateFallbackProfile();

		if (profile == null)
		{
			effectiveProfile.DisplayName = mercenaryName;
		}

		ApplyProfile(effectiveProfile);
		MercenaryName = effectiveProfile.DisplayName;
		GlobalPosition = spawnPosition;
		MoveSpeed = moveSpeed * Mathf.Max(0.25f, Profile.Stats.MoveSpeedMultiplier);
	...
	public void ApplyProfile(MercenaryProfile profile)
	{
		Profile = profile;
		MercenaryName = string.IsNullOrWhiteSpace(profile.DisplayName)
			? MercenaryName
			: profile.DisplayName;
		RefreshInventoryCapacity();

		if (Profile.Condition.Health <= 0)
		{
			Profile.Condition.Health = Profile.Stats.MaxHealth;
		}

		UpdateNameLabel();
	}

	public int GetCarryCapacity(int fallback)
	{
		return Profile.GetCarryCapacityFallback(fallback);
	}

	public void RefreshInventoryCapacity()
	{
		Inventory.MaxWeight = Mathf.Max(1.0f, Profile.GetCarryCapacityFallback(25));
	}

	public string GetInventoryWeightSummary()
	{
		return $"{Inventory.GetUsedWeight():0.#} / {Inventory.MaxWeight:0.#}";
	}

	public string GetInventoryContentsSummary()
	{
		return Inventory.GetContentsSummary();
	}

	public string GetLogisticsValidationWarning()
	{
		if (_baseBuildManager?.EnableLogisticsValidation != false)
		{
			ValidateLogisticsState(false);
		}

		return _logisticsValidationWarning;
	}

	public void ValidateLogisticsState(bool verboseLogs)
	{
		List<string> warnings = new();

		RefreshInventoryCapacity();

		if (!Inventory.Validate(out string inventoryWarning))
		{
		...
		if (_lifeAI != null)
		{
			string lifeWarning = _lifeAI.ValidateLogisticsState(this, verboseLogs);

			if (!string.IsNullOrWhiteSpace(lifeWarning))
			{
				warnings.Add(lifeWarning);
			}
		}

		string nextWarning = string.Join(" / ", warnings);

		if (!string.IsNullOrWhiteSpace(nextWarning)
			&& (verboseLogs || nextWarning != _logisticsValidationWarning))
		{
			GD.PushWarning($"Logistics validation warning for {MercenaryName}: {nextWarning}");
		}

		_logisticsValidationWarning = nextWarning;
	}

	public string GetCurrentWorkSummary()
	{
		return _lifeAI?.CurrentWorkSummary ?? "\uB300\uAE30";
	}

	public string GetCurrentWorkLabel()
	{
		return _lifeAI?.CurrentWorkLabel ?? "\uB300\uAE30";
	}

	public string GetCurrentWorkTargetLabel()
	{
		return _lifeAI?.CurrentTargetLabel ?? "\uC5C6\uC74C";
	}

	public string GetCurrentWorkStateLabel()
	{
		return _lifeAI?.CurrentStateLabel ?? "\uD560 \uC77C \uC5C6\uC74C";
	}

	public string GetCurrentWorkDecisionReason()
	{
		return _lifeAI?.CurrentDecisionReason ?? "\uAC00\uB2A5\uD55C \uC791\uC5C5 \uC5C6\uC74C";
	}

	public string GetCurrentUseProgressLabel()
	{
		return _lifeAI?.CurrentUseProgressLabel ?? "";
	}

	public string GetCurrentRoomEffectLabel()
	{
		return _lifeAI?.CurrentRoomEffectLabel ?? "";
	}

	public string GetConditionStatusSummary()
	{
		...
		if (condition.Mood <= 30.0f)
		{
			parts.Add("\uBD88\uB9CC");
		}

		return parts.Count == 0 ? "\uC591\uD638" : string.Join(" / ", parts);
	}
	...
	{
		string displayName = $"{MercenaryName} [{Profile.Rank}]\n{ControlMode}: {OrderState}";
		string workSummary = GetCurrentWorkSummary();
