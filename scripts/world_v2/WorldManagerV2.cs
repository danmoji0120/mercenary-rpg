...
public partial class WorldManagerV2 : Node
{
    ...
    public int CentralJobCount=>GameplaySessionV3.TryGetJobManager(out var jobs)&&jobs!=null?jobs.Count:0;public int QueuedCentralJobCount=>GameplaySessionV3.TryGetJobManager(out var jobs)&&jobs!=null?jobs.QueuedCount:0;public int ActiveCentralJobCount=>GameplaySessionV3.TryGetJobManager(out var jobs)&&jobs!=null?jobs.ActiveAssignmentCount:0;public long CentralJobAssignmentCount=>GameplaySessionV3.TryGetJobManager(out var jobs)&&jobs!=null?jobs.Diagnostics.AssignmentsSucceeded:0;public long CentralJobCandidateEvaluationCount=>GameplaySessionV3.TryGetJobManager(out var jobs)&&jobs!=null?jobs.Diagnostics.CandidateEvaluations:0;public string LastCentralJobAction=>GameplaySessionV3.TryGetJobManager(out var jobs)&&jobs!=null?jobs.Diagnostics.LastAction:string.Empty;
    public string SelectedMercenaryWorkPriorities{get{if(string.IsNullOrEmpty(MercenaryInspectHudDisplayedId)||!GameplaySessionV3.TryGetJobManager(out var jobs)||jobs==null)return "-";var p=jobs.GetOrCreatePriorityProfile(MercenaryInspectHudDisplayedId);return $"H{p.GetPriority(JobTypeV3.Hauling)} C{p.GetPriority(JobTypeV3.Construction)} D{p.GetPriority(JobTypeV3.Demolition)} G{p.GetPriority(JobTypeV3.Gathering)} S{p.GetPriority(JobTypeV3.Sowing)} V{p.GetPriority(JobTypeV3.Harvesting)}";}}
    ...
    private static void PrintCentralJobDetails()
    {
        if(!GameplaySessionV3.TryGetJobManager(out JobManagerV3? jobs)||jobs==null)return;
        int printed=0;foreach(JobRecordV3 job in jobs.GetJobsSnapshot())
        {
            if(job.State is not (JobStateV3.Queued or JobStateV3.Reserved or JobStateV3.Assigned or JobStateV3.Running or JobStateV3.RetryWaiting))continue;
            GD.Print($"  job={job.JobId} type={job.JobType} source={job.SourceKind}:{job.SourceId} state={job.State} priority={job.PriorityAtAssignment} target={job.TargetCell} merc={job.AssignedMercenaryId??"-"} work={job.WorkRequestId??"-"} retry={job.RetryAtSeconds:0.00} failure={job.FailureReason}");
            if(++printed>=12)break;
        }
    }
